...
# XClaw 服务管理脚本
# 用于安全地启动、停止和管理 XClaw 应用及 OpenClaw 服务
...
# 显示帮助信息
show_help() {
    echo -e "${BLUE}XClaw 服务管理脚本${NC}"
    echo ""
    echo "用法: ./$SCRIPT_NAME [命令]"
    echo ""
    echo "命令:"
    echo "  start     启动 XClaw 应用和 OpenClaw 服务"
    echo "  stop      安全停止 XClaw 和 OpenClaw 服务"
    echo "  restart   重启服务"
...
# 启动服务
start_service() {
    echo -e "${BLUE}正在启动 XClaw...${NC}"

    # 检查是否已经在运行
    if [ -f "$PID_FILE" ]; then
        local existing_pid=$(cat "$PID_FILE" 2>/dev/null)
        if [ -n "$existing_pid" ] && kill -0 "$existing_pid" 2>/dev/null; then
            echo -e "${YELLOW}⚠ XClaw 已经在运行 (PID: $existing_pid)${NC}"
