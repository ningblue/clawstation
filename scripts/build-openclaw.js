...
copyRecursive(path.join(OPENCLAW_SRC, 'dist'), path.join(RESOURCES_DIR, 'dist'));
copyRecursive(path.join(OPENCLAW_SRC, 'docs'), path.join(RESOURCES_DIR, 'docs'));
fs.copyFileSync(path.join(OPENCLAW_SRC, 'package.json'), path.join(RESOURCES_DIR, 'package.json'));
fs.copyFileSync(path.join(OPENCLAW_SRC, 'openclaw.mjs'), path.join(RESOURCES_DIR, 'openclaw.mjs'));

// 5. Install Production Dependencies
log('Installing production dependencies in resources...');
try {
  execSync('npm install --production --legacy-peer-deps', { cwd: RESOURCES_DIR, stdio: 'inherit' });
} catch (e) {
...
files.forEach(file => {
  ...
  patchFile(filePath, /(process[\w$]*)\.title\s*=\s*"openclaw"/g, '$1.title = $1.env.OPENCLAW_PROCESS_NAME || "openclaw"');
  ...
  patchFile(filePath, /process\.title\s*=\s*`\$\{cliName\}-\$\{name\}`/g, 'process.title = process.env.OPENCLAW_PROCESS_NAME || `\${cliName}-\${name}`');

  // Patch entry.js to force execution even when imported (via wrapper)
  // Regex: /if \(!isMainModule\(\{/g
  // Replacement: 'if (false && !isMainModule({'
  if (file === 'entry.js') {
    patchFile(filePath, /if \(!isMainModule\(\{/g, 'if (false && !isMainModule({');
  }
});
