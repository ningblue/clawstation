...
class ModelCatalogService {
  ...
  private getModelsJsonPath(): string {
    // OpenClaw 状态目录
    const openclawStateDir = process.env.OPENCLAW_STATE_DIR || path.join(os.homedir(), '.clawstation');
    // models.json 默认在 agent 目录下
    return path.join(openclawStateDir, 'agents', 'default', 'agent', 'models.json');
  }
  ...
  async loadModelCatalog(): Promise<ModelCatalogEntry[]> {
    ...
    try {
      ...
      if (!Array.isArray(modelArray)) {
        logger.warn('Invalid models.json format: expected array or { models: [...] }');
        return await this.loadStaticModelCatalog();
      }
      ...
      const models: ModelCatalogEntry[] = modelArray
        ...
        .map((entry: any) => {
          ...
          const provider = key.includes('/') ? key.split('/')[0] : String(entry.provider || '').trim();
          ...
          if (typeof entry.input === 'string') {
            input = entry.input.split('+').filter((i: string) => i === 'text' || i === 'image') as Array<'text' | 'image'>;
          } else if (Array.isArray(entry.input)) {
            input = entry.input.filter((i: string) => i === 'text' || i === 'image');
          }
  ...
  private async loadStaticModelCatalog(): Promise<ModelCatalogEntry[]> {
    logger.info('Attempting to load static model catalog from OpenClaw source...');

    // 尝试从 OpenClaw 的 pi-model-discovery 模块加载
    const possiblePaths = [
      path.join(__dirname, '../../../../lib/openclaw/dist/agents/pi-model-discovery.js'),
      path.join(__dirname, '../../../lib/openclaw/dist/agents/pi-model-discovery.js'),
      path.join(__dirname, '../../../../lib/openclaw/src/agents/pi-model-discovery.ts'),
    ];
    ...
    for (const modelPath of possiblePaths) {
      try {
        if (fs.existsSync(modelPath)) {
          ...
          // 尝试加载内置模型数据
          const builtinModelsPath = path.join(path.dirname(modelPath), 'models.json');
          if (fs.existsSync(builtinModelsPath)) {
            ...
            if (Array.isArray(data)) {
              const models = data
                ...
                .filter((entry: ModelCatalogEntry) => entry.id && entry.provider);
