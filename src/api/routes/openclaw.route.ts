...
export function setupOpenClawRoutes(openclawManager: OpenClawManager): void {
  ...
  // 添加/更新 Agent
  ipcMain.handle(
    "openclaw:config:agent:set",
    async (_event: IpcMainInvokeEvent, agent: any) => {
      try {
        const configManager = openclawManager.getConfigManager();
  ...
  // 设置默认 Agent
  ipcMain.handle(
    "openclaw:config:agent:setDefault",
    async (_event: IpcMainInvokeEvent, agentId: string) => {
      try {
        const configManager = openclawManager.getConfigManager();
  ...
  // 设置默认模型
  ipcMain.handle(
    "openclaw:config:defaultModel:set",
    async (_event: IpcMainInvokeEvent, model: any) => {
      try {
        const configManager = openclawManager.getConfigManager();
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        openclawManager.setApiKey(provider, apiKey, agentId, endpoint);
        return { success: true };
      } catch (error) {
  ...
  ipcMain.handle(
    "openclaw:apikey:has",
    async (_event: IpcMainInvokeEvent, provider: string, agentId?: string) => {
      try {
  ...
  // 获取指定提供商的所有模型
  ipcMain.handle(
    "openclaw:catalog:modelsByProvider",
    async (_event: IpcMainInvokeEvent, providerId: string) => {
      try {
        const models = await modelCatalogService.getModelsByProvider(
          providerId
        );
        return { success: true, models };
      } catch (error) {
  ...
  ipcMain.on(
    ...
    async (event: IpcMainEvent, message: string, conversationId?: number) => {
      try {
        if (!message || message.trim().length === 0) {
